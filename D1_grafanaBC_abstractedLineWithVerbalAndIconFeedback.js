// For humidity data. QUERY SELECT mean("humidity") FROM "hygrometer" WHERE ("deviceNo"::tag != '4') AND $timeFilter GROUP BY time(1h), "deviceNo" fill(null)
...
const series = context.panel.data.series.map((s, i) => {
  ...
  sData.map(d => {
    if (d > 60) outOfRangeCount++;
  })

  current = sData[sData.length - 1]
  let len = sTime.length;
  halftime = sTime[len - 10]
  height = Math.max(...sData);
  ...
  return {
    ...
    markPoint: {
      ...
      data: [
        {
          name: 'Mark',
          coord: [sTime[sTime.length - 15], height - 5],
          value: height,
          symbol: (current > 60 || current < 40) ? frown : smiley,
          itemStyle: {
            color: colors[i]
          }
        },
...
return {
  ...
  title: {
    text: 'Zu hoch für ' + outOfRangeCount + ' Std.',
    subtext: 'im ausgewählten Zeitraum ',
  },
  toolbox: {
    feature: {
      dataZoom: {
